...
int UNIFLoad(const char *name, FCEUFILE *fp) {
        ...
        /* Defensive: zero the buffer so any mapper access beyond the actual
         * PRG data reads predictable zeros instead of heap garbage. (The
         * primary fix for the COOLBOY 500-in-1 grey screen is the
         * prg_map_size computation below, but this guarantees no mapper
         * can ever read uninitialized memory from this buffer.) */
        memset(ROM, 0, UNIF_PRGROMSize);
        if (UNIF_CHRROMSize) {
                if (!(VROM = (uint8_t*)malloc(UNIF_CHRROMSize))) {
                        Cleanup();
                        return 0;
                }
                memset(VROM, 0, UNIF_CHRROMSize);
        }
        ...
        /* === FIX: 500-in-1 / COOLBOY grey-screen bug ===
         *
         * Background: FixRomSize() above rounded UNIF_PRGROMSize UP to the
         * next power of 2 (e.g. 33554688 B → 67108864 B) so the malloc()'d
         * ROM buffer is large enough. If we pass that rounded size to
         * SetupCartPRGMapping, PRGmask8 becomes too large (0x1FFF instead of
         * 0xFFF for a 32 MiB ROM). Mappers like COOLBOY (mapper 268) compute
         * a bank index such as 0x103F expecting the upper bits to be masked
         * away — with the wrong mask, bank 0x103F lands at byte offset
         * ~32.49 MiB, which is past the end of actual PRG data and inside
         * the uninitialized malloc() tail. The CPU then reads a garbage
         * reset vector and produces a permanent grey screen.
         *
         * Fix: tell the cart system the "usable" PRG size is the largest
         * power of 2 that fits within the actual PRG data. The buffer is
         * still allocated at UNIF_PRGROMSize (rounded up); only the mask
         * changes. Banks beyond the actual data are unreachable, which is
         * the intended behavior.
         */
        uint32_t prg_map_size = (uint32_t)UNIF_PRGROMSize;
        if ((uint32_t)prg_size_bytes < prg_map_size) {
            uint32_t p = 1;
            while (p * 2 <= (uint32_t)prg_size_bytes)
                p *= 2;
            prg_map_size = p;
        }
        SetupCartPRGMapping(0, ROM, prg_map_size, 0);
        if (UNIF_CHRROMSize)
