...
// running emulation thread. FBNeo's savestate code does NOT internally lock
// against retro_run(), so without this mutex, calling
// retro_serialize() / retro_unserialize() from the UI thread while the
// emulation thread is mid-retro_run() corrupts the FBNeo's internal state —
...
// around s_retro_run(); saveStateToPath/loadStateFromPath/flushSaveRamToDisk/
// reloadSaveRamFromDisk acquire it around their s_retro_serialize /
// s_retro_unserialize / SAVE_RAM I/O.
//
...
void stepFrame() {
    ...
    // FBNeo's frame update and corrupt internal state.
...
bool loadStateFromPath(int /*slot*/, const std::string& path) {
    ...
    // Hold the state mutex while s_retro_unserialize() restores FBNeo state.
    std::lock_guard<std::mutex> lk(s_stateMtx);
