...
// running emulation thread. FCEUmm's savestate code does NOT internally lock
// against retro_run(), so without this mutex, calling retro_serialize() /
// retro_unserialize() from the UI thread while the emulation thread is
// mid-retro_run() corrupts the NES's internal state — and the corrupted
...
void stepFrame() {
    ...
    // FCEUmm's frame update and corrupt internal state. The save/load side
    // uses a std::lock_guard on the same mutex; if the lock is held by the
    // emulation thread, the UI call blocks until the current frame finishes
    // (~16ms max) — perfectly acceptable latency for a manual save/load.
...
bool loadStateFromPath(int /*slot*/, const std::string& path) {
    ...
    // Hold the state mutex while retro_unserialize() restores NES state —
    // prevents retro_run() on the emulation thread from mutating state
    // mid-restore (which would either visibly corrupt the game or, in the
    // worst case, SIGSEGV from a half-restored pointer).
    std::lock_guard<std::mutex> lk(s_stateMtx);
...
bool flushSaveRamToDisk() {
    ...
    // SRAM buffer mid-copy, producing a torn .srm.
    std::lock_guard<std::mutex> lk(s_stateMtx);
