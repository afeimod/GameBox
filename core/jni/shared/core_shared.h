...
// ---------------------------------------------------------------------------
// 2xBR / 4xBR — Hyllian's original xBR algorithm (simple, fast)
//
// Original algorithm by Hyllian/Jararaca (2011). Uses integer color
// reduction (dot with [65536, 255, 1]) for exact pixel comparison.
// Only blends at detected edges: if (h==f && h!=e && edge_condition)
// then mix(E, F, 0.5). This is the simplest and fastest xBR variant.
//
// Fixes the dot artifacts of the old v3.3a by using exact integer
// comparison (no YUV threshold) — identical to the reference GLSL shader.
// ---------------------------------------------------------------------------

// Integer color reduction for exact pixel comparison (matches GLSL reduce())
static inline int64_t xbrReduce(uint32_t c) {
    int r = (c >> 16) & 0xFF;
    int g = (c >>  8) & 0xFF;
    int b =  c        & 0xFF;
    return (int64_t)r * 65536 + (int64_t)g * 256 + b;
}

// 2xBR upscale: src(sw × sh) → dst(2sw × 2sh)
static inline void xbr2xUpscale(const uint32_t* src, unsigned sw, unsigned sh,
    ...
    for (unsigned y = 0; y < sh; ++y) {
        for (unsigned x = 0; x < sw; ++x) {
            ...
            int64_t h = xbrReduce(H), i = xbrReduce(I);

            // Original 2xBR: all 4 sub-pixels get the same value
            // (the shader runs per-output-pixel, but the condition and result
            //  are the same for all sub-pixels within a source texel)
            uint32_t result = E;
            if (h==f && h!=e && (e==g && (h==i || e==d) || e==c && (h==i || e==b))) {
                uint32_t rE = (E >> 16) & 0xFF, gE = (E >> 8) & 0xFF, bE = E & 0xFF;
                uint32_t rF = (F >> 16) & 0xFF, gF = (F >> 8) & 0xFF, bF = F & 0xFF;
                result = 0xFF000000u | (((rE+rF+1)>>1) << 16) | (((gE+gF+1)>>1) << 8) | ((bE+bF+1)>>1);
            }

            unsigned x2 = x * 2, y2 = y * 2;
...
// 4xBR upscale: src(sw × sh) → dst(4sw × 4sh)
// Uses 4×4 sub-pixel pattern: E15 at corners, E11 at edges, E at center.
// This is the direct single-pass 4xBR — simpler and faster than two-pass cascade.
static inline void xbr4xUpscale(const uint32_t* src, unsigned sw, unsigned sh,
    ...
    for (unsigned y = 0; y < sh; ++y) {
        for (unsigned x = 0; x < sw; ++x) {
            ...
            // Default: all sub-pixels = E
            uint32_t E11 = E;  // edge blend (50% E + 50% F)
            uint32_t E15 = E;  // corner = F

            if (h==f && h!=e && (e==g && (h==i || e==d) || e==c && (h==i || e==b))) {
                // E11 = (E + F) / 2
                uint32_t rE = (E >> 16) & 0xFF, gE = (E >> 8) & 0xFF, bE = E & 0xFF;
                uint32_t rF = (F >> 16) & 0xFF, gF = (F >> 8) & 0xFF, bF = F & 0xFF;
                E11 = 0xFF000000u | (((rE+rF+1)>>1) << 16) | (((gE+gF+1)>>1) << 8) | ((bE+bF+1)>>1);
                E15 = F;
            }

            // 4x4 sub-pixel pattern (from original 4xBR shader):
            //   fp.x < 0.25:            fp.x < 0.50:            fp.x < 0.75:            fp.x >= 0.75:
            //   [E15 E11 E11 E15]       [E11 E  E  E11]         [E11 E  E  E11]         [E15 E11 E11 E15]
            unsigned x4 = x * 4, y4 = y * 4;
