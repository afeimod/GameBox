...
// running emulation thread. mGBA's savestate code does NOT internally lock
// against retro_run(), so without this mutex, calling
// retro_serialize() / retro_unserialize() from the UI thread while the
// emulation thread is mid-retro_run() corrupts the GBA's internal state —
...
void stepFrame() {
    ...
    // mGBA's frame update and corrupt internal state.
...
bool loadStateFromPath(int /*slot*/, const std::string& path) {
    ...
    // Hold the state mutex while retro_unserialize() restores GBA state.
    std::lock_guard<std::mutex> lk(s_stateMtx);
