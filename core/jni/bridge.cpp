...
void Engine::setPad2(int bits)        { rom::setControllerInput(1, (uint8_t)bits); }
...
JNIEXPORT void JNICALL
Java_com_nesstation_app_core_jni_NesNative_setPad2(JNIEnv*, jclass, jint bits) {
    nescore::Engine::instance().setPad2(bits);
}
...
Java_com_nesstation_app_core_jni_NesNative_saveState(JNIEnv* env, jclass, jint slot, jstring path) {
    const char* cpath = env->GetStringUTFChars(path, nullptr);
    bool ok = nescore::Engine::instance().saveState(slot, cpath ? cpath : "");
    if (cpath) env->ReleaseStringUTFChars(path, cpath);
    return ok ? JNI_TRUE : JNI_FALSE;
}
...
JNIEXPORT jboolean JNICALL
Java_com_nesstation_app_core_jni_NesNative_flushSaveRam(JNIEnv*, jclass) {
    return nescore::Engine::instance().flushSaveRam() ? JNI_TRUE : JNI_FALSE;
}
...
JNIEXPORT jboolean JNICALL
Java_com_nesstation_app_core_jni_NesNative_reloadSaveRam(JNIEnv*, jclass) {
    return nescore::Engine::instance().reloadSaveRam() ? JNI_TRUE : JNI_FALSE;
}
