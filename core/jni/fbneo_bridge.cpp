...
Java_com_nesstation_app_core_jni_FbNeoNative_saveState(JNIEnv* env, jclass, jint slot, jstring path) {
    const char* cpath = env->GetStringUTFChars(path, nullptr);
    bool ok = fbneocore::Engine::instance().saveState(slot, cpath ? cpath : "");
    if (cpath) env->ReleaseStringUTFChars(path, cpath);
    return ok ? JNI_TRUE : JNI_FALSE;
}
...
JNIEXPORT jboolean JNICALL
Java_com_nesstation_app_core_jni_FbNeoNative_flushSaveRam(JNIEnv*, jclass) {
    return fbneocore::Engine::instance().flushSaveRam() ? JNI_TRUE : JNI_FALSE;
}
...
JNIEXPORT jboolean JNICALL
Java_com_nesstation_app_core_jni_FbNeoNative_reloadSaveRam(JNIEnv*, jclass) {
    return fbneocore::Engine::instance().reloadSaveRam() ? JNI_TRUE : JNI_FALSE;
}
