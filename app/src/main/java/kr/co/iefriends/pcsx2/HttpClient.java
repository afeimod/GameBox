 ...
 * thread (see HTTPDownloaderAndroid.cpp phase-4.cpp) and calls into
 * {@link #doRequest} blocking-style. This class doesn't manage queues or
 * retries; it just runs ONE HTTP transaction and returns the result.
 * Caller is responsible for thread management and following any redirects
 * (we set {@code setInstanceFollowRedirects(true)} which covers the
 * rcheevos server's behaviour).</p>
 *
 * <p>IMPORTANT for GameBox: the emucore caches this class reference during
 * {@code initialize()} via {@code HTTPDownloaderAndroid::BindFromJNI}
 * ({@code FindClass("kr/co/iefriends/pcsx2/HttpClient")} +
 * {@code HttpClient$Response}). The lookup is soft-failing there (it logs
 * and returns false rather than aborting), but without this class the core
 * silently loses its HTTP downloader, so it must be present.</p>
 *
...
public final class HttpClient {
     ...
     * failure: -1 generic error, -2 timeout — matches the native
     * HTTPDownloader sentinels).
     *
