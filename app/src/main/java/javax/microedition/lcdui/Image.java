...
public class Image {
        ...
        void setSize(int width, int height) {
                if (width > bitmap.getWidth() || height > bitmap.getHeight()) {
                        ...
                        // Logical size shrunk — reallocate the Bitmap to the exact logical
                        // size to prevent stale pixels beyond (width, height) from causing
                        // ghost/duplicate images when the Bitmap is used as a GL texture
                        // or processed by pixel-level filters (XBR/HQ4x).
                        Bitmap b = bitmap;
                        bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
                        bounds.right = width;
                        bounds.bottom = height;
                        if (graphics != null) {
                                graphics.reset(0, 0, width, height);
                                // Copy only the valid (still-needed) region from the old bitmap
                                graphics.getCanvas().drawBitmap(b,
