...
{
	// Android bionic has no lockf(); use the equivalent POSIX fcntl() record
	// lock instead (lockf is itself a wrapper around fcntl advisory locks).
	struct flock fl = {};
	fl.l_type = F_WRLCK;
	...
	{
		Console.Error("fcntl(F_SETLKW) failed: %d", errno);
		m_fd = -1;
	}
}
